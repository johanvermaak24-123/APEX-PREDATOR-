...
let liveCount=0,waitCount=0,freshCount=0,missedCount=0,staleCount=0,invalidatedCount=0,unverifiedCount=0,missedCommandCount=0,tradeHold=0,tradeExit=0,tradeSwitch=0;
...
for(let i=0;i<N;i++){
  ...
  const expectedLive=!!(fire&&pass===true&&integrityPass===true&&execOK===true&&hydraPass&&geometry&&fr.pass&&(dir===1||dir===-1));
...
for(const stage of ['MISSED ENTRY','STALE SETUP','FRESHNESS BLOCK','HARD BLOCK','EXECUTION VETO','HYDRA VETO','PROOF ONLY','RESEARCH ARMED','WATCH','GEOMETRY BLOCK','WAIT']){
...
const report={version:'APEX-SCALP-GOD-7.3.0',sourceSha256:crypto.createHash('sha256').update(src).digest('hex'),scenarios:N,seed:'0x7300cafe',elapsedMs:Date.now()-started,passed:true,counts:{live:liveCount,nonLive:waitCount,fresh:freshCount,missed:missedCount,stale:staleCount,invalidated:invalidatedCount,unverified:unverifiedCount,missedUiAssertions:missedCommandCount,tradeHold,tradeExit,tradeSwitch},limits:{triggerFreshMaxMs:APEX_TRIGGER_FRESH_MAX_MS,commandMaxAgeMs:APEX_COMMAND_MAX_AGE_MS,maxTargetConsumed:0.35,maxFavorableATR:'dynamic 0.08–0.18 ATR'},invariants:['ENTER requires final authority AND explicit fresh-entry pass','Missing freshness metadata fails closed','Original trigger identity is tied to closed M5 bar, not rescan time','Price > fresh-zone / >35% target consumed becomes MISSED — DO NOT CHASE','Trigger older than 90s becomes STALE','Excess adverse displacement invalidates the old trigger','No entry/SL/TP levels leak while non-live','BEST NOW cannot create ENTER independently','Open-trade manager only emits HOLD / EXIT / SWITCH','Live command has a hard 60s maximum display life and may expire sooner with the trigger']};
